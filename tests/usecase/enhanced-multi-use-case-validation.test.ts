...
describe("Content validation across use cases", () => {
  const contentValidator = new ContentValidator();
  let tempDir: string;

  beforeAll(async () => {
    tempDir = mkdtempSync(join(tmpdir(), "content-usecase-"));
    await ensureDir(tempDir);
    await writeTextFile(join(tempDir, "report.md"), "# Analysis Report\n\nCompleted analysis.");
    await writeTextFile(join(tempDir, "data.csv"), "name,value\nfoo,1\nbar,2\n");
    await writeTextFile(
      join(tempDir, "code.ts"),
      "export function add(a: number, b: number) { return a + b; }",
    );
    await writeTextFile(join(tempDir, "config.yml"), "version: 1\ndebug: true\n");
    await writeTextFile(
      join(tempDir, "ui-component.tsx"),
      "export const Button = () => <button>Click</button>;",
    );
    await writeTextFile(join(tempDir, "draft.md"), "# Draft\n\nNewsletter content goes here.");
    await writeTextFile(
  ...
  const CONTENT_CASES: { type: UseCaseType; files: string[]; description: string }[] = [
    {
      type: "data-analysis",
      files: ["report.md", "data.csv"],
      description: "report and data files",
    },
    {
      type: "ui-design",
      files: ["ui-component.tsx", "report.md"],
      description: "UI component and report",
    },
    { type: "writing", files: ["draft.md"], description: "draft content" },
    { type: "devops", files: ["config.yml"], description: "config file" },
    { type: "coding", files: ["code.ts", "config.yml"], description: "code and config files" },
    { type: "testing", files: ["test.spec.ts", "report.md"], description: "test and report files" },
    { type: "documentation", files: ["report.md", "draft.md"], description: "documentation files" },
  ];

  for (const cc of CONTENT_CASES) {
    it(`should validate ${cc.type} content files: ${cc.description}`, async () => {
      const checks = await contentValidator.validateContent(tempDir, cc.files);
      expect(checks.length).toBe(cc.files.length);
  ...
  it("should fail when expected use case output file is missing", async () => {
    const checks = await contentValidator.validateContent(tempDir, [
      "missing-report.md",
      "missing-data.json",
    ]);
  ...
  it("should return evidence details for passed content checks", async () => {
    const checks = await contentValidator.validateContent(tempDir, ["code.ts"]);
    expect(checks[0]?.details).toBeDefined();
    expect(checks[0]?.details?.size).toBeGreaterThan(0);
  });
});
...
describe("Validation engine with content and file validation", () => {
  const engine = new ValidationEngine();
  let tempDir: string;

  beforeAll(async () => {
    tempDir = mkdtempSync(join(tmpdir(), "engine-usecase-"));
    await ensureDir(tempDir);
    await writeTextFile(
      join(tempDir, "analysis.md"),
      "# Analysis\n\nCompleted analysis with findings.",
    );
    await writeTextFile(join(tempDir, "visualization.png"), "fake-png-data");
    await writeTextFile(
      join(tempDir, "deploy-config.yml"),
      "environment: production\nregion: us-east-1\n",
    );
    await writeTextFile(
      join(tempDir, "test-results.xml"),
      "<testsuite><testcase name='test1'/></testsuite>",
    );
  });
  ...
  it("should validate content for data-analysis use case", async () => {
    const result = await engine.validateTask({
      projectRoot: tempDir,
      task: {
        ...baseTask,
        ...
        validation: { requiredContent: ["analysis.md"] },
      },
      executorResult: {
        status: "done",
        exitCode: 0,
        output: "Analysis report is saved as analysis.md with visualizations",
        startedAt: now(),
        finishedAt: now(),
      },
    });
    ...
    expect(contentChecks.length).toBe(1);
    expect(contentChecks[0]?.status).toBe("passed");
  });

  it("should validate both content and acceptance criteria for devops use case", async () => {
    const result = await engine.validateTask({
      projectRoot: tempDir,
      task: {
        ...baseTask,
        ...
        acceptanceCriteria: ["Deployment config is created", "Deployment validation passes"],
        validation: { requiredContent: ["deploy-config.yml"] },
      },
      executorResult: {
        status: "done",
        exitCode: 0,
        output: "Deployment config is created. Deployment validation passes.",
        startedAt: now(),
        finishedAt: now(),
      },
    });
    ...
    const criteriaChecks = result.checks.filter((c) => c.type === "acceptance_criteria");
    expect(criteriaChecks.length).toBe(2);
    for (const check of criteriaChecks) {
      expect(check.status).toBe("passed");
    }
  });

  it("should fail content validation when file is missing for writing use case", async () => {
    const result = await engine.validateTask({
      projectRoot: tempDir,
      task: {
        ...baseTask,
        ...
        title: "Write draft",
        acceptanceCriteria: [],
        validation: { requiredContent: ["missing-draft.md"] },
      },
      executorResult: {
        status: "done",
        exitCode: 0,
    ...
    expect(contentChecks[0]?.message).toContain("not found");
  });

  it("should validate file existence for testing use case", async () => {
    const result = await engine.validateTask({
      projectRoot: tempDir,
      task: {
        ...baseTask,
        ...
        runId: "run_content",
        title: "Run tests",
        acceptanceCriteria: ["Test results are saved"],
        validation: { requiredFiles: ["test-results.xml"] },
      },
      executorResult: {
        status: "done",
        exitCode: 0,
        output: "Test results are saved to test-results.xml",
        startedAt: now(),
        finishedAt: now(),
      },
    });
    ...
    expect(result.status).toBe("passed");
    const fileChecks = result.checks.filter((c) => c.type === "file");
    expect(fileChecks.length).toBe(1);
    expect(fileChecks[0]?.status).toBe("passed");
  });

  it("should fail file validation when expected test output is missing", async () => {
    const result = await engine.validateTask({
      projectRoot: tempDir,
      task: {
        ...baseTask,
        id: "task_missing_file",
        runId: "run_content",
        title: "Generate output",
        acceptanceCriteria: [],
        validation: { requiredFiles: ["missing-output.log"] },
      },
      executorResult: {
        status: "done",
        exitCode: 0,
    ...
    const fileChecks = result.checks.filter((c) => c.type === "file");
    expect(fileChecks[0]?.status).toBe("failed");
    expect(fileChecks[0]?.message).toContain("not found");
  });
});
